...
def start_phone(value):
    if value[0] != '0' and value[1] != '9':
        raise forms.ValidationError('شماره تلفن باید با 09 شروع شود')
    if len(value) > 11:
        raise forms.ValidationError(f"شماره تلفن باید 11 رقم باشد.طولش{len(value)}رقم است")


class LoginForm(forms.Form):
    phone = forms.CharField(required=True, widget=forms.TextInput(attrs={'class': 'form-control'}),
                            validators=[start_phone])
    password = forms.CharField(required=True, widget=forms.PasswordInput(attrs={'class': 'form-control'}),
                               validators=[validators.MinLengthValidator(8)])

    """
    به جای validators میشه از def clean رفت.هر دو یکی هستن ولی def clean قابل شخصی سازی هستش
